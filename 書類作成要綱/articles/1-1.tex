...
\checking{解決方法：括弧の対応関係を確認してください。}
\point{環境がおかしい}
\verb|\begin{}|と\verb|\end{}|が対応していなかったり、環境名が違っていたりするとエラーになります。
\checking{解決方法：環境の対応関係や名称を確認してください。}
\point{使用不可能な文字を使用している}
\LaTeX では\# や{\textbackslash}など、そのままでは出力できない半角文字（特殊文字）がいくつかあります。\footnotemark[2]
\checking{解決方法：「LaTeX 記号名」等で記号の出し方を検索する。該当する文字を直接使いたい場合は、{\textbackslash}verb{\textbar\textless} 記号{\textgreater} のようにする。}
\footnotetext[2]{\url{http://www.ic.daito.ac.jp/~mizutani/tex/special_characters.html}を参照するとよい。}
\point{Asosanに聞く}
Discordに投げていただければお答えできます。スクリーンショットとともに送ってください。

また\TeX はどの行でエラーになっているかがわからない（わかりにくい）場合があります。
その場合エラー箇所を探すのに苦労することになりますので、\boldwave{こまめに出力を行うようにしてください}。

\section{もっと\TeX を知りたい！}
今まで紹介してきた知識は\TeX のほんの断片的な情報に過ぎません。もっと詳しく\TeX について知りたい、あるいは数学式化学式を用いたレポートを
作成したいなどの場合には、「\boldtext{改訂第8版 - LaTeX2ε美文書作成入門（奥村 春彦先生著）}」を適宜参照するとよいでしょう。
